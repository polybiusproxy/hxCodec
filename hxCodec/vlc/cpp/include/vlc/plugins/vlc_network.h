...
#define IN6_IS_ADDR_MULTICAST(a) (((__const uint8_t *)(a))[0] == 0xff)
    ...
    static inline int net_GetSockAddress(int fd, char *address, int *port)
    {
        ...
        return getsockname(fd, (struct sockaddr *)&addr, &addrlen) || vlc_getnameinfo((struct sockaddr *)&addr, addrlen, address,
    ...
    static inline int net_GetPeerAddress(int fd, char *address, int *port)
    {
        ...
        return getpeername(fd, (struct sockaddr *)&addr, &addrlen) || vlc_getnameinfo((struct sockaddr *)&addr, addrlen, address,
                                                                                      NI_MAXNUMERICHOST, port, NI_NUMERICHOST)
                   ? VLC_EGENERIC
                   : 0;
    }
